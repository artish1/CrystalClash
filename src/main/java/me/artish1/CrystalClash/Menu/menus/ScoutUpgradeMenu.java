...
public class ScoutUpgradeMenu implements Menu,Listener{
	private String name;
	private Menu parent;
	private int size;
	...
	public ScoutUpgradeMenu(String name,int size, JavaPlugin plugin,Menu parent) {
		this.name = name;
		this.parent = parent;
		this.size = size;
	...
	public ScoutUpgradeMenu addItem(int index, MenuItem item){
		items[index] = item;
		items1.add(item);
	...
	public void apply(Inventory inventory,Player p){
		items1.clear();
		Material boots = MySQLUtil.getBoots(p.getUniqueId(), ClassType.SCOUT);
		Material helmet = MySQLUtil.getHelmet(p.getUniqueId(), ClassType.SCOUT);
		Material sword = MySQLUtil.getItem(p.getUniqueId(), ClassType.SCOUT, "sword");
		Material leggings = MySQLUtil.getLeggings(p.getUniqueId(), ClassType.SCOUT);
		
		addItem(13, new HelmetItem(helmet,ClassType.SCOUT,p.getUniqueId()));
	    addItem(22 + 18, new UpgradeBootsItem(boots,ClassType.SCOUT,p.getUniqueId()));
	    addItem(3 + 18, new UpgradeSwordItem(sword,ClassType.SCOUT,p.getUniqueId()));  
		addItem(13 + 18, new UpgradeLeggingsItem(leggings, ClassType.SCOUT ,p.getUniqueId()));
