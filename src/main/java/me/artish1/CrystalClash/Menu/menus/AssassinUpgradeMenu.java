...
import me.artish1.CrystalClash.Menu.items.UpgradeAssassinSwordPoisonItem;
...
public class AssassinUpgradeMenu implements Menu,Listener{
	private String name;
	private Menu parent;
	private int size;
	...
	public AssassinUpgradeMenu(String name,int size, JavaPlugin plugin,Menu parent) {
		this.name = name;
		this.parent = parent;
		this.size = size;
	...
	private void registerItems(Player p)
	{
		
		
		addItem(13, new UpgradeAssassinSwordPoisonItem(p.getUniqueId()));
	...
	public AssassinUpgradeMenu addItem(int index, MenuItem item){
		items[index] = item;
		items1.add(item);
